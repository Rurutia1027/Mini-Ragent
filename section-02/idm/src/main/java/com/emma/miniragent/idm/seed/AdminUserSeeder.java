...
public class AdminUserSeeder implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        if (userRepository.existsByUsername(ADMIN_USERNAME)) {
            // admin already exists in db, no need to load seed script
            LOGGER.info("Seed user '{}' already exists", ADMIN_USERNAME);
            return;
        }

        userRepository.insert(ADMIN_USERNAME, passwordEncoder.encode(ADMIN_PASSWORD));
        LOGGER.info("Seeded default user '{}' / '{}'", ADMIN_USERNAME, ADMIN_PASSWORD);
    }
}
