...
public class AuthService {
    ...
    public LoginResponse login(String username, String password) {
        ...
        User user = userRepository.findByUsername(username.trim())
                .orElseThrow(() -> new ClientException("Username or Password Wrong!"));
        if (!passwordEncoder.matches(password, user.getPasswordHash())) {
            throw new ClientException("Username or Password Wrong!");
        }
    ...
    public String requireUsername(String token) {
        if (token == null || token.isBlank()) {
            throw new ClientException("User not login or Login Expired");
        }
    ...
    private void validateCredentials(String username, String password) {
        ...
        if (username.trim().length() < 3) {
            throw new ClientException("Username at least 3 characters");
        }
