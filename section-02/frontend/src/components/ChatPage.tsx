...
export default function ChatPage({token, username, onLogout, onUnauthorized}: ChatPageProps) {
    ...
    async function handleSubmit(event: FormEvent) {
        ...
        try {
            ...
            if (!response.ok || !response.body) {
                // Auth interceptor may returnJSON Result on ClientException
    ...
    return (
        <div className="page">
            <header className="header header-row">
                <div>
                    <h1>Mini Ragent</h1>
                    <p>Section 2 . Auth + Local Ollama SSE</p>
                </div>
            ...
            <main className="chat-panel">
                {messages.length === 0 && (
                    <div className="empty">Input Question, Ollama Will Give You Answer</div>
                )}
            ...
            <form className="composer" onSubmit={handleSubmit}>
                <textarea
                    ...
                    placeholder="Input Message, and Submit"
                    rows={3}
                    disabled={loading}
                    onKeyDown={(e) => {
